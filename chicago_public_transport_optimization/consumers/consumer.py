...
class KafkaConsumer:
    ...
    def __init__(
        ...
        if is_avro is True:
            ...
            self.consumer = AvroConsumer({"bootstrap.servers": self.broker_properties['BROKER_URL'], "group.id": "0"},
                        schema_registry=self.broker_properties["schema.registry.url"],)
        else:
            self.consumer = Consumer({"bootstrap.servers": self.broker_properties['BROKER_URL'], "group.id": "0"})
